...
def calculateOdometry(currentPos, leftEnc, rightEnc, rearEnc, frontEnc): #Encoder measurements are assumed to be in ticks
    leftEnc = leftEnc / 336857.5 #From ticks to meters
    rightEnc = rightEnc /336857.5
    rearEnc = rearEnc / 158891.2
    frontEnc = frontEnc / 158891.2
    ...
    while not rospy.is_shutdown():
        if newSpeedData:
            ...
            if not simulated:
                ...
                frontSpeed = int(frontSpeed*16.0/35.0*127)
                rearSpeed = int(rearSpeed*16.0/35.0*127)
                #print "lS: " + str(leftSpeed) + " rS: " + str(rightSpeed) + " fS: " + str(frontSpeed) + " rS: " + str(rearSpeed)
                try:
                    if leftSpeed >= 0:
                        Roboclaw1.BackwardM1(address2, leftSpeed)
                    else:
                        Roboclaw1.ForwardM1(address2, -leftSpeed)

                    if rightSpeed >= 0:
                        Roboclaw1.ForwardM2(address2, rightSpeed)
                    else:
                        Roboclaw1.BackwardM2(address2, -rightSpeed)

                    if frontSpeed >= 0:
                        Roboclaw2.ForwardM2(address1, frontSpeed)
                    else:
                        Roboclaw2.BackwardM2(address1, -frontSpeed)

                    if rearSpeed >= 0:
                        Roboclaw2.ForwardM1(address1, rearSpeed)
                    else:
                        Roboclaw2.BackwardM1(address1, -rearSpeed)
                except:
            ...
            if speedCounter < -1:
                speedCounter = -1
        if not simulated:
            a1, encoderLeft, a2 = Roboclaw1.ReadEncM1(address2)
            #print Roboclaw2.ReadEncM2(address2)
            #print Roboclaw2.ReadEncM1(address2)
            #print Roboclaw1.ReadEncM1(address1)
            #print Roboclaw1.ReadEncM2(address1)
            b1, encoderRight, b2 = Roboclaw1.ReadEncM2(address2) #The negative sign is just because it is the way the encoders are wired to the roboclaw
            c1, encoderRear, c2 =  Roboclaw2.ReadEncM2(address1)
            d1, encoderFront, d2 = Roboclaw2.ReadEncM1(address1)
            ...
            encoderLeft = leftSpeed * 0.05 * 336857.5
            encoderRight = rightSpeed * 0.05 * 336857.5
            encoderFront = frontSpeed * 0.05 * 158891.2
            encoderRear = rearSpeed * 0.05 * 158891.2
