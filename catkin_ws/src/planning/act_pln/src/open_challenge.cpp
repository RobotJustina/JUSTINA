...
void callbackCmdExplainThePlan(const planning_msgs::PlanningCmdClips::ConstPtr& msg){
	...
	if(success2){
        ...
        JustinaHRI::waitAfterSay("I am going to explain the plan", 1000);
				explain = true;
			}
			else{
...
void callbackCmdAnswer(const planning_msgs::PlanningCmdClips::ConstPtr& msg){
	...
	if(success){
		...
		if(param1.compare("a_question") == 0){
			...
			if(success){
        success = JustinaHRI::waitAfterSay("I am waiting for the user question", 1000);
				planning_msgs::planning_cmd srv;
      ...
      JustinaHRI::waitAfterSay("of the national autonomous university of mexico", 1000);
		}
...
void callbackCmdWorld(const planning_msgs::PlanningCmdClips::ConstPtr& msg){
	...
	if(success){
        ...
        JustinaHRI::waitAfterSay("I am going to search persons in the scene", 1000);
        ...
        JustinaHRI::waitAfterSay("John is in the left", 1000);
			...
			else if(arthurCI != arthurCD && arthurCI < arthurCD && robert > 0){
        ...
        JustinaHRI::waitAfterSay("john is in the right", 1000);
          ...
          std::cout << "John esta SOLO" << std::endl;
					ss << "john solo";
          JustinaHRI::waitAfterSay("john is the only person I can see", 1000);
				}
				else
					ss << "john nil";
			}
        ...
        JustinaHRI::waitAfterSay("Peter is in the left", 1000);
			...
			else if(robertCI != robertCD && robertCI < robertCD && arthur > 0)
			{
        ...
        JustinaHRI::waitAfterSay("Peter is in the right", 1000);
			...
			else
			{
				if(robert>0)
				{
					...
					ss << " peter solo";
          JustinaHRI::waitAfterSay("Peter is the only person I can see", 1000);
				}
				else
				{
					ss << " peter nil";
      ...
      JustinaHRI::waitAfterSay("I am going to search objects on the table", 1000);
        ...
        found = JustinaVision::detectObjects(recognizedObjects);
			...
			if(chocosyrup>0){						
				responseObject.params ="coffe table";
        JustinaHRI::waitAfterSay("coffe is on the table", 1000);
				command_response_pub.publish(responseObject);
				objectsids.push_back("coffe");
			}
			else
			{
				responseObject.params ="coffe nil";
				command_response_pub.publish(responseObject);
			}

			if(coconutmilk>0){						
				responseObject.params ="stevia table";
        JustinaHRI::waitAfterSay("The stevia is on the table", 1000);
				command_response_pub.publish(responseObject);
				objectsids.push_back("stevia");
			}
			else
			{
				responseObject.params ="stevia nil";
				command_response_pub.publish(responseObject);
			}

			if(coke>0){						
				responseObject.params ="soup table";
        JustinaHRI::waitAfterSay("The soup is on the table", 1000);
				command_response_pub.publish(responseObject);
      ...
      JustinaTasks::sayAndSyncNavigateToLoc("inspection", 120000);
			...
			if(srv.response.args == "what_see_yes"){
        JustinaHRI::waitAfterSay("I am ready for another question", 1000);
			}
...
void callbackCmdDescribe(const planning_msgs::PlanningCmdClips::ConstPtr& msg)
{
    ...
    JustinaHRI::waitAfterSay("There are no persons in the room", 1000);
		...
		if(tokens[3] == "nil" && tokens[1]!= "nil"){
      JustinaHRI::waitAfterSay("peter is the only person in the room", 1000);
      ...
      JustinaHRI::waitAfterSay("john is the only person in the room", 1000);
  ...
  JustinaHRI::setNodeHandle(&n);
	...
	while(ros::ok()){
		switch(state){
			case SM_INIT:
				if(startSignalSM){
          JustinaHRI::waitAfterSay("Hellow my name is Justina, I'm ready for the open chanlenge", 1000);
