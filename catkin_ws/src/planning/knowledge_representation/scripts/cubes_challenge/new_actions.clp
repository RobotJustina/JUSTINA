...
(defrule plan_explain_cubes_plan
	...
	(assert (plan (name ?name) (number 5)(actions put_on_top_simul ?block1 ?block2)))
	(assert (plan (name ?name) (number 6)(actions update_status original nil)))
	(assert (plan (name ?name) (number 7)(actions restore_stacks)))
	(assert (plan (name ?name) (number 8)(actions restore_cubes blue_block)))
	(assert (plan (name ?name) (number 9)(actions restore_cubes red_block)))
	(assert (plan (name ?name) (number 10)(actions restore_cubes green_block)))
	(assert (plan (name ?name) (number 11)(actions update_status stack_exp explained)))
	(assert (finish-planner ?name 11))
)
