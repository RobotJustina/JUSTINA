...
(defrule plan_explain_cubes_plan
	...
	(assert (plan (name ?name) (number 6)(actions put_on_top_simul ?block1 ?block2)))
	(assert (plan (name ?name) (number 7)(actions enable_simul False)))
	(assert (plan (name ?name) (number 8)(actions update_status original nil)))
	(assert (plan (name ?name) (number 9)(actions restore_stacks)))
