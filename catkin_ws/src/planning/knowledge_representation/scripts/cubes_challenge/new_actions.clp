...
(defrule plan_explain_cubes_plan
	...
	(bind ?speech1(str-cat "I explained the plan, Now i start to execute the command"))
	(bind ?confirmation(str-cat "Explain_the_plan"))
	(assert (plan (name ?name) (number 1)(actions confirmation ?confirmation)))
	(assert (plan (name ?name) (number 2)(actions make_task ?name) (actions_num_params 3 19)))
	(assert (plan (name ?name) (number 3)(actions speech-anything ?speech)))
	(assert (plan (name ?name) (number 4)(actions backup_cubes blue_block)))
	(assert (plan (name ?name) (number 5)(actions backup_cubes red_block)))
	(assert (plan (name ?name) (number 6)(actions backup_cubes green_block)))
	(assert (plan (name ?name) (number 7)(actions enable_simul True)))
	(assert (plan (name ?name) (number 8)(actions go_to_place table)))
	(assert (plan (name ?name) (number 9)(actions put_on_top_simul ?block1 ?block2)))
	(assert (plan (name ?name) (number 10)(actions go_to_place dining_room)))
	(assert (plan (name ?name) (number 11)(actions enable_simul False)))
	(assert (plan (name ?name) (number 12)(actions update_status original nil)))
	(assert (plan (name ?name) (number 13)(actions restore_stacks)))
	(assert (plan (name ?name) (number 14)(actions restore_cubes blue_block)))
	(assert (plan (name ?name) (number 15)(actions restore_cubes red_block)))
	(assert (plan (name ?name) (number 16)(actions restore_cubes green_block)))
	(assert (plan (name ?name) (number 17)(actions reset_cube_pos blue_block)))
	(assert (plan (name ?name) (number 18)(actions reset_cube_pos red_block)))
	(assert (plan (name ?name) (number 19)(actions reset_cube_pos green_block)))
	(assert (plan (name ?name) (number 20)(actions speech-anything ?speech1)))
	(assert (plan (name ?name) (number 21)(actions update_status stack_exp explained)))
	(assert (finish-planner ?name 21))
)
