...
(defrule plan_explain_cubes_plan
	...
	(assert (plan (name ?name) (number 2)(actions make_task ?name stack_exp explained) (actions_num_params 3 19)))
	...
	(assert (plan (name ?name) (number 11)(actions enable_simul True)))
	(assert (plan (name ?name) (number 12)(actions update_status original nil)))
	(assert (plan (name ?name) (number 13)(actions restore_stacks_dynamic delate)))
