...
(defrule plan_explain_cubes_plan
	...
	(assert (plan (name ?name) (number 3)(actions make_task ?name stack_exp explained) (actions_num_params 4 19)))
	...
	(assert (plan (name ?name) (number 20)(actions restore_stacks_dynamic delate)))
	(assert (plan (name ?name) (number 21)(actions speech-anything ?speech1)))
	(assert (plan (name ?name) (number 22)(actions update_status stack_exp explained)))
	(assert (finish-planner ?name 22))
)
