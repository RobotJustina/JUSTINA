...
class JustinaTasks
{
    ...
    public:
        ...
        static bool waitRecognizedSkeleton(float timeout, std::vector<vision_msgs::Skeleton> &skeletons);
        static bool findPerson(std::string person = "", int gender = -1, POSE pose = NONE, bool recogByID = false, std::string location = "");
        static bool findSkeletonPerson(std::string location = "");
    ...
    private:
        ...
        static bool turnAndRecognizeSkeleton(float initAngPan, float incAngPan,float maxAngPan, float initAngTil, float incAngTil, float maxAngTil,float incAngleTurn, float maxAngleTurn, float maxDistance, Eigen::Vector3d &centroidSkeleton, std::string location);
